...
class makeDijetHists(processor.ProcessorABC):
    ...
    def __init__(self, ptcut = 30., etacut = 2.5, data = False):
        ...
        mass_bin = hist.axis.Regular(30, 0, 1000.,name="mreco", label="Jet Mass (GeV)")
        mass_gen_bin = hist.axis.Regular(30, 0, 1000., name="mgen", label="Gen Jet Mass (GeV)")
        ...
        self._histos = {
            ...
            'misses_pt_m':       hist.Hist(dataset_cat, pt_gen_bin, mass_gen_bin, storage="weight", name="Events"),
            'fakes_pt_m':        hist.Hist(dataset_cat, pt_bin, mass_bin, storage="weight", name="Events"),
...
#             sel.add("gen_asymm_sel", asymm_gen_sel)
            ...
            misses = ~matches
            out["misses_pt_m"].fill(dataset=dataset, ptgen = ak.flatten(events[misses].GenJetAK8.pt), 
...
#         sel.add("allsel_reco", allsel_reco)
        ####  Final RECO plots
        dijets = events.FatJet[:,0]+events.FatJet[:,1]
        ...
        out["jet_pt_mass_reco_g"].fill( dataset=dataset, ptreco=dijets.pt, mreco=dijet_msd, weight=weights )
        ...
        if self.do_gen:
            ...
            out["fakes_pt_m"].fill(dataset = dataset, ptreco = ak.flatten(events[fakes].FatJet.pt),

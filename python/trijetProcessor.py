...
class makeTrijetHists(processor.ProcessorABC):
    def __init__(self, ptcut = 200., ycut = 2.5, btag = 'None', data = False, jet_systematics = ['nominal', 'JER',"HEM"], systematics = ['L1PreFiringWeight', 'PUSF'], jk=False, jk_range = None):
        ...
        rho_bin = hist.axis.Regular(20, -5.0, 0.0, name="rhoreco", label=r"$\rho$")
        rho_gen_bin = hist.axis.Regular(10, -5.0, 0.0, name="rhogen", label=r"$\rho$")
        ...
        if not self.jk:
            ...
            'asymm_reco':                   hist.Hist(dataset_axis, syst_cat, frac_axis, storage="weight", label="Events"),
            'asymm_gen':                    hist.Hist(dataset_axis, syst_cat, frac_axis, storage="weight", label="Events"),
    ...
    def process(self, events):
        ...
        datastr = events.metadata['dataset'][:20]
        ...
        for jk_index in jk_inds:
            ...
            print("corrected fat jets ", corrected_fatjets)
            ...
            for jetsyst in jet_corrs.keys():
                ...
                if self.do_gen:
                    ...
                    if not self.jk:
                        out["asymm_gen"].fill(dataset=datastr, syst=jetsyst, frac = asymm_gen[sel.all("triGenJet_seq")], weight=weights[sel.all("triGenJet_seq")])
                ...
                if not self.jk:
                    ...
                    out["asymm_reco"].fill(dataset=datastr, syst=jetsyst, frac = asymm[sel.all("triRecoJet_seq")], weight=weights[sel.all("triRecoJet_seq")])
                ...
                if self.do_gen:
                    ...
                    if ak.sum(miss_sel) > 0:
                        ...
                        out["misses"].fill(dataset=datastr, syst=jetsyst, jk=jk_index, ptgen = miss_jets[~ak.is_none(miss_jets.mass)].pt, mgen = miss_jets[~ak.is_none(miss_jets.mass)].mass, weight = miss_weights[~ak.is_none(miss_jets)])
                    ...
                    if len(weights[fake_sel])>0:
                        ...
                        #                        weightDown=q2Down) 
                        out["fakes"].fill(dataset=datastr, syst=jetsyst, jk = jk_index, ptreco = fake_jets[~ak.is_none(fake_jets.mass)].pt, mreco = fake_jets[~ak.is_none(fake_jets.mass)].mass, weight = fake_weights.weight()[~ak.is_none(fake_jets.mass)])
                ...
                if self.do_gen:
                    ...
                    #### Outliers
                    if not self.jk:
                        out["jet_eta_phi"].fill(dataset=datastr, syst=jetsyst, phi=jet.phi, rapidity=getRapidity(events_corr.FatJet[:,2].p4), weight=self.weights[jetsyst].weight())

...
void testUnfold()
{
    ...
    TTreeReader myReader("response", uprootFile);
    // The branch "reco_gen_groomed" contains doubles; access them as response_matrix_g
    TTreeReaderArray<Double_t> response_matrix_g(myReader, "reco_gen_groomed");
    // The branch "reco_gen_ungroomed" contains doubles; access them as response_matrix_u
    TTreeReaderArray<Double_t> response_matrix_u(myReader, "reco_gen_ungroomed");
    
    int n_reco = 0;
    
    while (myReader.Next()) {
        cout << "Size of groomed: " << response_matrix_g.GetSize() << " response_ungroomed: " << response_matrix_u.GetSize() << endl;
        n_reco++;
        cout << "Number of reco bins" << n_reco << endl;
    }
...
//         cout << "Bin edge " << massBins_fine[i-1] << endl;;
//     }
    TUnfoldBinning *recoBins = new TUnfoldBinning(*mreco_mgen_u->GetXaxis(), true, true);
    recoBins->AddAxis(*ptreco_ptgen_u->GetXaxis(), true, true);
    cout << recoBins << endl;
    
    TUnfoldBinning *genBins = new TUnfoldBinning(*mreco_mgen_u->GetXaxis(), true, true);
    genBins->AddAxis(*mreco_mgen_u->GetYaxis(), true, true);
    cout << &genBins << endl;
